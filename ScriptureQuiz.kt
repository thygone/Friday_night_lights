...
class ScriptureQuiz : AppCompatActivity() {
    ...
    val scriptures = listOf(
        ...
        mapOf(
            ...
            "chapter" to "2",
            "verses" to "27",
            "text" to "Wherefore, men are free according to  the flesh; and all things are given them which are expedient unto  man. And they are free to  choose liberty and eternal life, through the great Mediato r of all men, or to  choose captivity and death, according to  the captivity and power of the devil; for he seeketh that all men might be miserable like unto  himself.",
            "speaker" to "Lehi"
        ),
        mapOf(
            "book" to "Mosiah",
    ...
    fun displayAnswers(vol: String?) {
        ...
        var books: Array<String?> = arrayOfNulls(14) // the posible books to show
        ...
        books.shuffle() // randomize the order of the books
        for (i in (1..3)) {// add them to the books
            var chap = Math.abs(Random.nextInt() % 60)
            var ver = Math.abs(Random.nextInt() % 60) // 60 will be the highest verse we will use
            answers[i] = (books[i] + " " + chap + ":" + ver)
        }
